...
# =============================================================================
# --- 전역 실행 설정 변수 ---
TARGET_URL_BASE = "https://www.jejusi.go.kr/information/intro/news.do?currentPageNo="
MAX_PAGES_TO_CRAWL = 5
TARGET_YEAR_TO_EXTRACT = "all"

# --- 엑셀 파일 저장 설정 변수 ---
OUTPUT_FILENAME = "제주도 기관 공고.xlsx"
TARGET_SHEET_NAME = "제주시청"
START_CELL_FOR_DATA = "B6"
# "링크" 항목을 마지막 열로 이동
EXCEL_HEADER = ["번호", "제목", "부서", "첨부", "작성일", "조회수", "링크"]
# =============================================================================


def crawl_jejusi_notices_lxml(url_base, max_pages, target_year):
    """
    제주시청 공지사항 목록에서 특정 년도의 자료를 추출합니다.
    lxml과 XPath를 사용하며, 작성일 기준으로 특정 년도의 공고만 추출합니다.
    """
    # --- 크롤링할 요소의 XPath 표현식을 변수로 정의 ---
    TABLE_CONTAINER_XPATH = "//*[@id='contents']/div/div[3]"  # 변경된 테이블 컨테이너 XPath
    ITEM_ROW_XPATH = "./table/tbody/tr"  # 테이블 컨테이너 안의 테이블 구조를 반영
    NUMBER_XPATH = "./td[1]"             # 번호
    subject_XPATH = "./td[2]/a"             # 제목  <- 수정됨
    DEPT_XPATH = "./td[3]"              # 부서
    ATTACHMENT_XPATH = "./td[4]/img"         # 첨부
    DATE_XPATH = "./td[5]"              # 작성일
    HIT_XPATH = "./td[6]"               # 조회수
    TITLE_LINK_XPATH = "./td[@class='title']/a" # 링크 (분리 필요)

    results = []
    date_formats_to_try = ["%Y-%m-%d", "%y-%m-%d", "%Y.%m.%d", "%y.%m.%d", "%Y/%m/%d", "%y/%m/%d", "%Y년%m월%d일", "%Y. %m. %d"]
    ...
    for page in range(1, max_pages + 1):
        ...
        try:
            ...
            table_container = tree.xpath(TABLE_CONTAINER_XPATH)
            if not table_container:
                print("테이블 컨테이너를 찾을 수 없습니다.")
                break
            table_container = table_container[0]
            rows = table_container.xpath(ITEM_ROW_XPATH)
            ...
            for row in rows:
                try:
                    ...
                    # 각 행 내부에서 제목 요소 찾기
                    subject_element = row.xpath(subject_XPATH)[0] if row.xpath(subject_XPATH) else None
                    title = subject_element.text.strip() if subject_element is not None else ""

                    dept_element = row.xpath(DEPT_XPATH)[0] if row.xpath(DEPT_XPATH) else None
                    dept = dept_element.text.strip() if dept_element is not None else ""

                    attachment_element = row.xpath(ATTACHMENT_XPATH)[0] if row.xpath(ATTACHMENT_XPATH) else None
                    attachment = "있음" if attachment_element is not None else ""
                    ...
                    title_link_element = row.xpath(TITLE_LINK_XPATH)[0] if row.xpath(TITLE_LINK_XPATH) else None
                    link = urljoin(target_url.split('?')[0], title_link_element.get('href')) if title_link_element is not None and title_link_element.get('href') else ""
                    ...
                    if date:
                        if target_year == "all" or date.year == int(target_year) if target_year != "all" else True:
                            # 헤더 순서에 맞춰 데이터 추가
                            results.append([number_str, title, dept, attachment, date_str, hit_str, link])
                    else:
                        print(f"날짜 형식 인식 오류: 작성일={date_str}")
...
if __name__ == "__main__":
    crawled_data = crawl_jejusi_notices_lxml(TARGET_URL_BASE, MAX_PAGES_TO_CRAWL, TARGET_YEAR_TO_EXTRACT)
    if crawled_data:
